...
def _to_numpy_array(x, dtype=None):
    # Unwrap 0-d object arrays from npz pickles
    if isinstance(x, np.ndarray) and x.dtype == object and x.ndim == 0:
        x = x.item()

    # Convert scipy sparse or similar
...
class Dataset:
    def __init__(self, name, add_self_loops=False, device='cpu', use_sgc_features=False, use_identity_features=False,
        ...
        base = os.path.join('data', f'{name.replace("-", "_")}')
        npz_path = base + '.npz'
        ...
        if os.path.exists(npz_path):
            ...
            npz_base = base+'.npz'
            data = np.load(os.path.join('data', base, "raw", npz_base), allow_pickle=True)
        node_features = torch.from_numpy(_to_numpy_array(data['node_features'], np.float32))
        labels = torch.from_numpy(_to_numpy_array(data['node_labels']))
        edges = torch.from_numpy(_to_numpy_array(data['edges'], np.int64))
        ...
        train_masks = torch.from_numpy(_to_numpy_array(data['train_masks'], np.bool_))
        val_masks = torch.from_numpy(_to_numpy_array(data['val_masks'], np.bool_))
        test_masks = torch.from_numpy(_to_numpy_array(data['test_masks'], np.bool_))
